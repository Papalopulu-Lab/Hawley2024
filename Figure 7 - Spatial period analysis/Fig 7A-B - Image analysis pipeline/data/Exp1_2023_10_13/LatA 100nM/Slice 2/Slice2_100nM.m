...
x_image_plot = linspace(0, left_h*dx, size(roi_left, 1));
y_image_plot = linspace(0, left_h*dx, size(roi_left, 2));

figure(2), subplot(241), imagesc(roi_left), title('Left side'), axis equal, xlim([0 size(roi_left,2)]);
% set(gca, 'XTick', x_image_plot)
