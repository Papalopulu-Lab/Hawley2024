function [spatialPeriodsInd, spatialPeriodsComb, interpFrequency, powerAvg, pk2pk_combined, detrendedSDcomb, coherence, pearsonPlot] = plotResults(allResults, condName, plotGraphs)
    ...
    for nExp = 1:numExp
        ...
        for nCon = 1:numConditions
            ...
            if contains(allResults(nExp).name, condName(nCon))
                ...
                pearsonCombined{1, nCon} =  [pearsonCombined{nCon}; allResults(nExp).pearson];
            end
        end
    end
...
for n = 1:N
    ...
    normalityTest(2, n) = swtest(allPowerSpectra{2}(:,n));
        ...
        if plotGraphs == 1
            ...
            b1 = powerIQR(1, 2:end);
            b2 = powerIQR(2, 2:end);
            boundedline(x, y1, b1, 'cmap', plotColours, 'transparency', 0.2, 'alpha', 'linewidth', 1.5)
            xlim([0 200])
            ylim([0 max(powerAvg, [], 'all')*2.1])
             % ylim([0 4.5e-4])
            xlabel('Spatial period (um)')
...
figure(1032)
clf
pearsonPlot = [pearsonCombined{1}; pearsonCombined{2}];
violinplot(pearsonPlot, expNameInd, 'QuartileStyle','shadow', 'ShowMean', true, 'ViolinColor', plotColoursExps);
ylabel('Spatial period (\mum)')
set(gca, 'FontSize', fontSize)
title('Fourier with Fisher-G')
